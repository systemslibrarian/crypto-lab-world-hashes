 ...
 *    an unknown backdrop, or a `color-mix()`. Almost nothing on this page sits on
 *    a flat colour. `<body>` carries TWO radial-gradient washes (`--hero-glow` at
 *    12% 2% and `--amber-soft` at 88% 14%), and every `.panel` and `.card` — which
 *    is every exhibit, every explainer and every table on the page — is painted
 *    with `linear-gradient(180deg, var(--surface-elevated),
 *    var(--surface-elevated-2))` between two TRANSLUCENT `rgba()` surfaces, so
 *    the washes show through the panels. On top of that `--accent-soft`,
 *    `--amber-soft`, `--chip` and `--hash-block` are all `rgba()`: they are the
 *    surface under the active tab, under every `.callout`, under every changed
 *    nibble in the avalanche view, under every `.prop-card`, and under every
 *    64-hex `.digest-block`. The hero aside is a
 *    `color-mix(in oklab, var(--accent) 6%, transparent)` and the shared top bar's
 *    `--cl-ink` is a `color-mix(in srgb, ...)`. axe files all of them under
 ...
 *    which is not the colour that lands on screen. This page had three such
 *    rules and two of them were defects, found by this walk rather than by axe:
 *    the `<footer>`'s inline `opacity: 0.7` dragged the related-demo links down
 *    to 3.04:1 in the light theme, and `.avalanche-ideal`'s `opacity: 0.55` put
 *    the 50% marker at 1.35:1 over its own meter fill. Both are fixed at the
 *    source rather than by tuning the number, because no opacity value fixes an
 *    opacity defect. What is left is `.cl-hero-sub { opacity: .85 }`, which
 *    composites to 11.80:1 dark and 10.28:1 light.
 *
 ...
 *    boxes are the four `.compare-table-wrap` scrollers (`overflow-x: auto`
 *    around tables that take `min-width: 700px` below 860px) and
 *    `.avalanche-meter` (`overflow: hidden`). At 380px each of those tables
 *    genuinely scrolls, so most of the known-answer table, the resistance table
 *    and both reference tables are outside the frame. Content scrolled past them
 *    is not dimmed or partly drawn — it is absent, and asking what colour it sits
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This branch
 *    is load-bearing here rather than inherited. The lab draws three real
 *    labelled figures in SVG — the Merkle-Damgard chain, the sponge, and the
 *    wide-pipe chain — and every one of them is `<text>` sitting on top of a
 *    `<rect>` sibling: `.mech-msg` over `--accent-soft`, `.mech-f` and
 *    `.mech-perm` over `--panel-strong`, `.mech-cap` over `--amber-soft`,
 *    `.mech-digest` over a translucent green. An ancestor-only walk would
 *    composite those labels onto the `<svg>`'s own transparent background instead
 *    and report ratios nothing on screen has. Note also that SVG's initial `fill`
 *    is BLACK, which `getComputedStyle` reports for stroke-only geometry too —
 *    so the `.mech-line` connectors, which are `fill: none` paths, are excluded
 *    from the underlay by the fill test below rather than read as opaque black
 *    rectangles crossing the labels.
 *
 * THREE THINGS THIS WALK CANNOT SEE, stated so neither is mistaken for coverage.
 *
 ...
 * `styles.css` and the shared header's inline block between them contain no
 * `content` declaration at all, and no `::before` or `::after` selector of any
 * kind. `nontext.ts` runs the generated-content check at every driven state
 * regardless, so the first one added is measured rather than assumed.
 *
 * SECOND, CSS `filter`. The walk composites colours; it does not run the filter
 * pipeline. This page has one: `.attack-button:hover { filter: brightness(1.08) }`,
 * which lightens both the button's `--accent` fill and its ink together. The walk
 * therefore reports the unhovered ratio for that state. It is an OVER-report of
 * about 0.65 here rather than a miss — hand-measured, the hovered light-theme
 * button is #ffffff on rgb(0,110,220), 4.92:1, against the 5.57:1 the walk
 * reports, and the hovered dark-theme button stays far above the floor. Recorded
 * so the gap is a known quantity rather than a surprise the first time a filter
 * is used to DARKEN something.
 *
 * THIRD, `title` attributes. The inline glossary is built from
 * `<abbr class="gloss" tabindex="0" title="...">`, and the definition is
 * delivered by the UA's own tooltip — chrome this page does not paint and cannot
 * style, so there is nothing here to measure. It is recorded because the dotted
 * underline that CUES the tooltip is measurable and is measured: it is drawn with
 * `text-decoration-color: var(--accent)`, which the walk sees as part of the
 * `.gloss` element's own text run.
 */
 ...
 * uses. Nothing here is short-lived enough to need the narrowed form: the only
 * `@keyframes` in the stylesheet is `live-pulse` on `.live-dot`, which the
 * reduced motion this gate asserts clamps to a single 0.01ms iteration, and
 * `.avalanche-fill`'s width transition drains before `settle()` returns. The one
 * genuinely transient rendering is the 1200ms "Copied" relabel on a copy
 * control, and that is long enough to scan whole.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * are still in the DOM behind the `hidden` attribute — plus a seventeen-row
     ...
     * that is the four `.compare-table-wrap` scrollers and `.avalanche-meter`
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="explainer">` disclosures, all of which ship shut and are
      // rebuilt shut by every `render()`. The gate opens each by clicking its
      // <summary>, which is the route a reader has, rather than setting `.open`
      // from script — which is what the gate this replaces did to all of them at
      // once, before its single scan. It also matters for the five INACTIVE TAB
      // PANELS, which the old gate un-hid wholesale.
      ...
      // `top:-3rem`, and this lab's own `.skip-link` parks at `left:-9999px`.
      // Measuring either parked copy invents a failure for text that is not on
      // screen; both focused renderings are real states and the gate scans each
      ...
      //
      // `styles.css` DEFINES `.sr-only` with exactly that recipe
      // (`position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)`) but
      // nothing in `src/main.ts` currently uses the class, so the guard is
      // inert on this page today. It is kept, and the fact that it is inert is
      // recorded here rather than left to be rediscovered, because the recipe is
      // already sitting in the stylesheet waiting to be applied: the first
      // `.sr-only` span added would otherwise be measured as a 1.x:1 failure for
     ...
     *
     * What this lab hides is a closed set of SIX elements and every one of them
     * is an empty `<span>` with no text node at all: the five `.live-dot`s that
     * precede each "recomputes live" note, and the `.avalanche-ideal` marker at
     * the 50% point of the avalanche meter. There is nothing for this walk to
     * measure inside any of them, so the exclusion costs this page nothing.
     * (The `.avalanche-ideal` marker is still a 1.4.11 case even though it holds
     * no text; it is measured by hand and its numbers are in `styles.css`.)
     *
     * Nothing on this page hides a VALUE. Every digest, every avalanche
     * percentage, every attack verdict and every known-answer result is exposed
     ...
     *
     * This lab has three real SVG figures, so the guard is live rather than
     * inherited — and the way the mistake arrives is exactly how this lab builds
     * its DOM: the entire page is `innerHTML` template literals, and every one of
     * those diagrams is interpolated as a string. Interpolating a string ARRAY
     * into a template makes JS join it with commas, leaving a run of ","
     ...
     *
     * It is NOT a no-op here, and this page is close to the exact shape that
     * breaks: `styles.css` opens with `html, body { min-height: 100% }`, one
     * keyword away from the `height: 100%` that caused the original failure. The
     * background is also not a flat colour — `<body>` carries two
     * radial-gradient washes over `--bg` — so the root paint has to be SAMPLED
     * rather than read, which is why this uses `paintAt` rather than
     * `backgroundColor`. In the dark theme that surface is `#081018`; falling
     * through to white instead would fabricate a failure for every element on a
     * page that runs six exhibits deep.
     */
