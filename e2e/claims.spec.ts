...
test('LSH is hashed live, and its hand-rolled maths is pinned to two sources', async ({
  ...
  await expect(lshRow).toContainText('Wikipedia "LSH (hash function)" reference digest');
  ...
  // recomputes the whole shipped vector set on load — and reports the two kinds
  // SEPARATELY, because Crypto++'s vectors were generated by the very
  // implementation this lab transcribed from. Totalling them would present
  // cross-implementation agreement as conformance to the standard.
  ...
  const [, independent, crossImpl] =
    note.match(/(\d+) independent · (\d+) matched to Crypto\+\+/) ?? [];
  expect(Number(independent)).toBeGreaterThanOrEqual(6);
  expect(Number(crossImpl)).toBeGreaterThan(0);
  // The two must not be silently merged back into one figure.
  expect(note).not.toMatch(/\d+\/\d+ published LSH vectors/);

  // The prose states what each source is actually worth, and no longer claims
  // the two corroborate each other past the three bytes where they overlap.
