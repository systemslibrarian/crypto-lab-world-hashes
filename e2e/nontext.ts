 ...
 * check that could not have found anything. `styles.css` defines
 * `--control-border` as the boundary token and applied it to exactly two rules:
 * `textarea, input[type='text'], select`, and `.attack-button.secondary`. Every
 * other control drew its edge from `--border`, the SURFACE divider used for
 * `.panel`, `.digest-block` and table rules — `.tab-button`, every
 * `.button-row button`, every `.copy-btn`, and all 61 `.flip-cell`s. Measured
 * against the surface outside them: 1.36-1.52:1 in dark, 1.42-1.54:1 in light.
 *
 * The spec that was supposed to catch this, `e2e/border.spec.ts`, is deleted,
 * and it failed twice over. It queried `textarea, input[type='text'], select` —
 * precisely and only the three selectors `--control-border` was already applied
 * to, so it asserted 3:1 over the rules that already kept it. And it compared
 * each element's `borderTopColor` against its OWN `backgroundColor`, never
 * against the surface outside the control, so it did not ask the question
 * 1.4.11 asks: whether the control can be told apart from what surrounds it.
 *
 * The second check finds nothing here, and that is a measurement rather than an
 * omission: neither `styles.css` nor the shared header's inline block contains a
 * single `content` declaration or any `::before`/`::after` selector. It runs at
 * every driven state so the first one added is caught.
 *
