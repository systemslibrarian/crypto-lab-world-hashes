...
/** Message length in bytes, whatever form the vector spells it in. */
const messageLength = (v: (typeof LSH_VECTORS)[number]): number =>
...
describe('published LSH vectors', () => {
  ...
  it('carries independently-published vectors, not only self-generated ones', () => {
    // Crypto++'s LSH test data was generated by Crypto++'s own implementation —
    // the one this file's constants were transcribed from. Checking against it
    // proves the transcription is faithful and nothing more, so the table has
    // to carry digests published by someone else as well.
    const independent = LSH_VECTORS.filter((v) => v.independent);
    expect(independent.length).toBeGreaterThanOrEqual(6);
    expect(new Set(independent.map((v) => v.source)).size).toBeGreaterThanOrEqual(2);
  });

  it('anchors the multi-block path to an INDEPENDENT source, not just Crypto++', () => {
    // This is the guard that matters. A wrapper that is wrong only past one
    // block is the exact failure the bundled Bash package shipped, and an
    // over-block vector generated by the implementation under test would not
    // catch it if that implementation were the one at fault.
    for (const width of [28, 32] as const) {
      const independent = LSH_VECTORS.filter((v) => v.independent && v.digestBytes === width);
      const lengths = independent.map(messageLength);
      expect(
        lengths.some((n) => n > LSH256_BLOCK_BYTES),
        `LSH-256-${width * 8}: no independent vector longer than one block`,
      ).toBe(true);
      // And the exact-multiple case, which forces a whole extra padding block.
      expect(
        lengths.some((n) => n > 0 && n % LSH256_BLOCK_BYTES === 0),
        `LSH-256-${width * 8}: no independent vector at an exact block multiple`,
      ).toBe(true);
    }
  });
...
describe('block-boundary coverage', () => {
  it('spans the 128-byte block in both directions, for both widths', () => {
    for (const digestBytes of [28, 32] as const) {
      const lengths = LSH_VECTORS.filter((v) => v.digestBytes === digestBytes).map(messageLength);
