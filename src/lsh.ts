 ...
 * Agency (KISA)".
 *
 * Transcription is exactly where a hand-rolled primitive goes wrong, so
 * `crossCheckLsh()` re-runs the whole shipped table on every page load and
 * every test run. What matters is WHO published each expected digest:
 *
 *   1. **Independent** — Korea's own algorithm-validation vectors (the Korean
 *      analogue of NIST's CAVP ShortMsg/LongMsg files), read from
 *      RyuaNerin/go-krypto at commit 478a2e9a667e15f6fa42bc4457cf7e2e5798d6b7
 *      and pinned by blob hash below, plus the `"abc"` reference digests
 *      published in Wikipedia's "LSH (hash function)" article. Neither comes
 *      from Crypto++. These cover the empty message, exactly one block, and
 *      past one block, for both widths.
 *   2. **Cross-implementation** — Crypto++'s own LSH test data. Its file header
 *      says `Source: gen_lsh256.cpp`: it was GENERATED by the implementation
 *      these constants were transcribed from. It proves the transcription is
 *      faithful; it cannot independently prove the standard is met. The page
 *      and the KAT badge count it separately for that reason.
 *
 * That distinction is not pedantry. A wrapper that is correct below one block
 * and wrong above it is precisely what the bundled Bash package shipped, and a
 * vector generated by the implementation under test cannot catch that class of
 * bug in the implementation that generated it. So the multi-block path is
 * anchored to the independent set, and a unit test asserts it stays that way.
 *
 * During development the full Crypto++ set (294 vectors, 0 to 65,536 bytes) and
 * the full Korean validation set (266 vectors across both widths) were both run
 * against this file; all 560 matched. The subset embedded here is what ships,
 * so the page can re-check its own arithmetic without carrying a 560-vector
 * table.
 */
...
export interface LshVector {
  ...
  source: string;
  /**
   * Was this digest published by someone other than the implementation this
   * file was transcribed from? Computed coverage depends on it, so it is a
   * field rather than a string match on `source`.
   */
  independent: boolean;
}

/**
 * Crypto++'s LSH test data. Its file header reads `Source: gen_lsh256.cpp`,
 * i.e. these were GENERATED by Crypto++'s own implementation — the same
 * implementation this file's constants were transcribed from. They prove the
 * transcription is faithful; they cannot independently prove either side
 * matches the standard. Marked `independent: false` for exactly that reason.
 */
const CRYPTOPP = "Crypto++ LSH test data — generated by Crypto++'s own implementation";

/** The published "abc" reference digests, which do not come from Crypto++. */
const WIKIPEDIA = 'Wikipedia, "LSH (hash function)" — published reference digest';

/**
 * Korea's own algorithm-validation vectors — the Korean analogue of NIST's
 * CAVP files, down to the ShortMsg/LongMsg naming.
 *
 * Transcribed from RyuaNerin/go-krypto, an independent Go implementation of the
 * Korean standards that cites them. The criteria document itself was not
 * fetched, and the label says so rather than implying otherwise.
 *
 * Pinned to the exact bytes read, because repositories move:
 *
 *   repo    github.com/RyuaNerin/go-krypto
 *   commit  478a2e9a667e15f6fa42bc4457cf7e2e5798d6b7  (2024-01-22)
 *   files   lsh256/testcases_256_test.go
 *             blob 12ac37415e176da2897fde947698fed0aecf7e0d
 *           lsh256/testcases_224_test.go
 *             blob 061d3ce7ae092bf6679cbd8acc066e8e18dd5f5f
 *
 * Those blob hashes are `git hash-object` of the files as read, and were
 * checked against the GitHub API at that commit. Anyone can re-derive them:
 *   git cat-file blob 12ac3741… | sha1sum   (or fetch the path at that commit)
 * If upstream rewrites history or the content drifts, the hashes stop matching
 * and this provenance claim fails loudly rather than rotting quietly.
 */
const KCMVP =
  '암호알고리즘 검증기준 V3.0 (Korean algorithm-validation criteria), ' +
  'LSH(256-*) Short/LongMsg — via github.com/RyuaNerin/go-krypto ' +
  'lsh256/testcases_{224,256}_test.go @ 478a2e9';

/**
 * The shipped vector set.
 *
 * Independently-published digests first — the `"abc"` pair, the empty message,
 * exactly one block, and 227 bytes (past one block) — then Crypto++'s
 * cross-implementation set filling in 127 bytes and the larger exact multiples.
 * Both widths get independent coverage on each side of the block boundary.
 */
export const LSH_VECTORS: LshVector[] = [
  { label: 'LSH-256-256, "abc"', digestBytes: 32, message: { kind: 'text', value: 'abc' }, expected: '5fbf365daea5446a7053c52b57404d77a07a5f48a1f7c1963a0898ba1b714741', source: WIKIPEDIA, independent: true },
  { label: 'LSH-256-224, "abc"', digestBytes: 28, message: { kind: 'text', value: 'abc' }, expected: 'f7c53ba4034e708e74fba42e55997ca5126bb7623688f85342f73732', source: WIKIPEDIA, independent: true },
  { label: 'LSH-256-256, empty message', digestBytes: 32, message: { kind: 'zeros', length: 0 }, expected: 'f3cd416a03818217726cb47f4e4d2881c9c29fd445c18b66fb19dea1a81007c1', source: KCMVP, independent: true },
  { label: 'LSH-256-224, empty message', digestBytes: 28, message: { kind: 'zeros', length: 0 }, expected: '48a0d55b2b3d91f26e06f7110fe9ce8ea0e2656bbe344cb1c5930653', source: KCMVP, independent: true },
  { label: 'LSH-256-256, 127-byte message (one short of a block)', digestBytes: 32, message: { kind: 'hex', value: '4736cec0425fa500d737100811587619e431b753f3886333d65d2be10b1643b30b0156fc7257df9933c35d48af2fa7ea4337ac3ba6605f450aadea9e5369631a835aa2a59260563a2788e1459faf3f12e92b683f0d714b158e2e63a58d6c842da058ff23cfd6e8787572cd1147c55859fbeaabcf592641df5d3167321e8f70' }, expected: '82c7c18f22f752725ce8c29364e5f87c4be0294c484dc363474885ba8aab38dd', source: CRYPTOPP, independent: false },
  ...
  { label: 'LSH-256-256, 128 zero bytes (exactly one block)', digestBytes: 32, message: { kind: 'zeros', length: 128 }, expected: 'd44fdd8a41c88053f3409a7c298fb6f51ab8e3add1f3412440313cdef54005a4', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-256, 256 zero bytes', digestBytes: 32, message: { kind: 'zeros', length: 256 }, expected: '2e8608025f4f89b9393173933d232b4c2ee8300a6442c7f653c601df01f62f81', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-256, 384 zero bytes', digestBytes: 32, message: { kind: 'zeros', length: 384 }, expected: '242c96d54d672959339a977fe543cb07a856a50d9f3bce264746a03dda7021e7', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-256, 512 zero bytes', digestBytes: 32, message: { kind: 'zeros', length: 512 }, expected: '10d3cc85aee4fe6bf90abdce4945776902f857e34c1d1ea3a043ca5fa16eb1d1', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-224, 128 zero bytes (exactly one block)', digestBytes: 28, message: { kind: 'zeros', length: 128 }, expected: 'ba4793d003dc5d4de44ebfbecb1430a13a69f41664d04436984e4cf3', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-224, 256 zero bytes', digestBytes: 28, message: { kind: 'zeros', length: 256 }, expected: '779b665fa8c905836c92821802ad9b83170d978d3e4245321963631a', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-224, 384 zero bytes', digestBytes: 28, message: { kind: 'zeros', length: 384 }, expected: 'ea79cca2aa02223d91d965fd275e07a13bf0aeb9a9c4790335fd4b50', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-224, 512 zero bytes', digestBytes: 28, message: { kind: 'zeros', length: 512 }, expected: '80ff3f526f5a5b273abdbb6b14b296b63563b488834c58b802cff38b', source: CRYPTOPP, independent: false },
  { label: 'LSH-256-256, 128-byte message (exactly one block)', digestBytes: 32, message: { kind: 'hex', value: '' }, expected: '2049e9305ed6e0d9e0cfba6a267f7983fa4886cbe544499e8c21e8ffd7a08daa', source: KCMVP, independent: true },
  { label: 'LSH-256-256, 227-byte message (over one block)', digestBytes: 32, message: { kind: 'hex', value: '' }, expected: 'eac621cd6495672ef57f82d2fbe852db5ab7a7a8775188eeed5b39cec355044e', source: KCMVP, independent: true },
  { label: 'LSH-256-224, 128-byte message (exactly one block)', digestBytes: 28, message: { kind: 'hex', value: '' }, expected: '5a409d2c7b9cbc4e45db8e468184fbc57239706f9b9264c0b3dcb399', source: KCMVP, independent: true },
  { label: 'LSH-256-224, 227-byte message (over one block)', digestBytes: 28, message: { kind: 'hex', value: '' }, expected: '7617289f423459cd2dea4e69ad71ba0ee17022b75f9f1532d3c4aaf5', source: KCMVP, independent: true },
];
...
export interface LshCheck {
  label: string;
  source: string;
  /** Carried through from the vector so the UI can count the two kinds apart. */
  independent: boolean;
  computed: string;
  published: string;
  agrees: boolean;
