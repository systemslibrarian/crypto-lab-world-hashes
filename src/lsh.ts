 ...
 *      analogue of NIST's CAVP ShortMsg/LongMsg files), plus the published
 *      `"abc"` reference digests. Neither comes from Crypto++. These cover the
 *      empty message, exactly one block, and past one block, for both widths.
 ...
 * CAVP files, down to the ShortMsg/LongMsg naming. Transcribed here from
 * RyuaNerin/go-krypto, an independent Go implementation that cites them; the
 * criteria document itself was not fetched, and the label says so.
 */
const KCMVP =
  '암호알고리즘 검증기준 V3.0 (Korean algorithm-validation criteria), ' +
  'LSH(256-*) Short/LongMsg — via RyuaNerin/go-krypto';
